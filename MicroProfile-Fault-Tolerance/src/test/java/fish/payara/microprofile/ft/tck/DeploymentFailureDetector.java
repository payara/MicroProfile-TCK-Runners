 ...
 * archive fails to deploy (for example when a Fault Tolerance annotation is
 * misconfigured and the FT CDI extension throws a
 * {@code FaultToleranceDefinitionException}), Micro still boots, reports an
 * empty {@code "Deployed": []} list and stays running. The connector therefore
 * returns success and never propagates the failure, which makes tests annotated
 * with {@code @ShouldThrowException(FaultToleranceDefinitionException.class)}
 * fail with "no exception was thrown".
 *
 * <p>Payara Micro logs the failure to {@code System.out} (the connector's
 * {@code OutputLoggingConsumer} pipes the child process output there when
 * console output is enabled, which is the default). This observer captures that
 * output around each deployment and, if Micro logged a deployment failure,
 * rethrows it as a {@link DeploymentException}. That exception is raised while
 ...
 *
 * <p>The observer only touches Arquillian SPI, so it compiles and registers on
 * every profile. On the full-server/embedded profiles a failed deployment
 * throws during {@code deploy()} (before {@code AfterDeploy} fires) and the
